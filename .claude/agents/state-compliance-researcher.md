...
Always check the local PostgreSQL database at `localhost:13247` BEFORE making Jina API calls. This saves API costs and speeds up generation for previously researched states.
...
**Schema for `state_compliance` table:**
```sql
state_code TEXT,              -- e.g., "TX"
state_name TEXT,              -- e.g., "Texas"
service_niche TEXT,           -- e.g., "Title Loans"
is_legal BOOLEAN,             -- Whether service is legal
loan_type_allowed TEXT,       -- Description of allowed model
restrictions TEXT,            -- State-specific restrictions
notes TEXT,                   -- Additional notes
regulations JSONB,            -- {maxAPR, allowsRollover, etc.}
fees JSONB,                   -- Fee caps and structures
licensing JSONB,              -- Licensing requirements
consumer_protections JSONB,   -- Rights and protections
city_page_content JSONB,      -- Pre-written city page content
research_sources JSONB,       -- Array of research sources
updated_at TIMESTAMP,
PRIMARY KEY (state_code, service_niche)
```
...
**Before ANY Jina API calls**, check the local database for existing compliance data.

#### 0a. Check for Cached State Compliance
```bash
# Check if state compliance data exists for target state + niche
PGPASSWORD=password psql -h localhost -p 13247 -U postgres -d cities_db -t -A -c \
  "SELECT state_code, service_niche FROM state_compliance WHERE state_code = 'CA' AND service_niche = 'Title Loans';"
```

#### 0b. Get Full Compliance Data for a State
```bash
# Get complete compliance data as JSON
PGPASSWORD=password psql -h localhost -p 13247 -U postgres -d cities_db -t -A -c \
  "SELECT row_to_json(t) FROM (SELECT * FROM state_compliance WHERE state_code = 'CA' AND service_niche = 'Title Loans') t;"
```

#### 0c. List All Cached States for a Niche
```bash
# See which states already have compliance data
PGPASSWORD=password psql -h localhost -p 13247 -U postgres -d cities_db -t -A -c \
  "SELECT state_code, state_name FROM state_compliance WHERE service_niche = 'Title Loans' ORDER BY state_code;"
```

#### 0d. Decision Logic
```
FOR EACH target state:
  IF state + niche exists in state_compliance:
    → USE cached data (skip Jina)
    → Log: "CACHE HIT: [state_code] [service_niche]"
  ELSE:
    → RESEARCH via Jina (Steps 1-4)
    → SAVE to database (Step 4b)
    → Log: "CACHE MISS: [state_code] - researching..."
```

**Cache-First Benefits:**
- Zero API cost for previously researched states
- Instant data retrieval vs. web scraping delays
- Consistent compliance data across projects
...
### Step 4b: SAVE TO DATABASE CACHE (After Jina Research)

**After researching a state via Jina, ALWAYS save to the database for future use:**

```bash
# Insert or update state compliance data
PGPASSWORD=password psql -h localhost -p 13247 -U postgres -d cities_db -c "
INSERT INTO state_compliance (
  state_code, state_name, service_niche, is_legal, loan_type_allowed,
  restrictions, notes, regulations, fees, licensing, consumer_protections,
  city_page_content, research_sources, updated_at
)
VALUES (
  'CA',
  'California',
  'Title Loans',
  true,
  'Title Loans (Regulated)',
  'Subject to California Financing Law',
  'California has some of the strictest consumer lending regulations',
  '{\"maxAPR\": \"30%\", \"allowsRollover\": false, \"maxLoanAmount\": \"$10,000\"}'::jsonb,
  '{\"maxOriginationFee\": \"Included in APR cap\", \"maxLatePaymentFee\": \"5%\"}'::jsonb,
  '{\"required\": true, \"licenseType\": \"CFL\", \"regulatoryBody\": \"DFPI\"}'::jsonb,
  '{\"rightToRescind\": {\"available\": true, \"period\": \"2 days\"}}'::jsonb,
  '{\"headline\": \"Understanding Title Loan Laws in California\", \"keyPoints\": [...]}'::jsonb,
  '[\"California DFPI\", \"CFPB\"]'::jsonb,
  NOW()
)
ON CONFLICT (state_code, service_niche) DO UPDATE SET
  is_legal = EXCLUDED.is_legal,
  regulations = EXCLUDED.regulations,
  fees = EXCLUDED.fees,
  licensing = EXCLUDED.licensing,
  consumer_protections = EXCLUDED.consumer_protections,
  city_page_content = EXCLUDED.city_page_content,
...
**This enables:**
- Future projects skip Jina for this state + niche
