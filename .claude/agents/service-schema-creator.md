...
## 🚨 MANDATORY: USE JINA API - NEVER WRITE FROM MEMORY

### ❌ YOU MUST NEVER:

| Forbidden Action | Why It's Wrong |
|-----------------|----------------|
| Write `service-schema-template.json` from memory | Your training data is outdated/generic |
| Generate services list without Jina research | Results in incomplete/incorrect offerings |
| Skip Step 1 (Jina research) | Results in generic content |
| Copy example JSON verbatim | Examples are templates, not actual output |

### ✅ YOU MUST:

1. **FIRST** run Jina `curl` commands to research the service niche (Step 1)
2. **FETCH** at least 2-3 competitor websites via `r.jina.ai`
3. **COMPILE** services list from actual research results
4. **ONLY THEN** write `service-schema-template.json`

### 🔍 VERIFICATION:

Before writing the schema file, verify:
- You ran at least 3 `curl` commands with Jina API
- Services list is based on research (not invented)
- FAQs are based on research (not generic)

**If you wrote the schema without running curl first, DELETE IT and start over.**
...
**CRITICAL: CTA URL by Niche Type:**
| Niche Type | ctaUrl | Reason |
|------------|--------|--------|
| **YMYL (Lending)**: Title Loans, Payday Loans, Personal Loans | `/apply` | Uses ApplyWizard with Salesforce iframe |
| **YMYL (Medical)**: Medical Services | `/apply` or `/schedule` | Booking/intake forms |
| **YMYL (Legal)**: Legal Services | `/apply` or `/consultation` | Consultation requests |
| **Non-YMYL**: Plumber, Electrician, HVAC, Roofing | `/contact` | Simple contact form |
...
ANTI-DOORWAY NOTE (see docs/ANTI-DOORWAY.md):
- 30 locations → 30 city pages (NOT 300!)
- One city = ALL services
```

This schema generates pillar pages and provides service list for city pages.
...
- **nextjs-orchestrator**: Uses pillar schema for /services/[service] pages
