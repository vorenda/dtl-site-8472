...
## 🚨 MANDATORY: READ INPUT FILES FIRST - NEVER GENERATE FROM MEMORY

### ❌ YOU MUST NEVER:

| Forbidden Action | Why It's Wrong |
|-----------------|----------------|
| Write city pages from memory | Your training data is generic, triggers doorway penalties |
| Invent landmarks | Must come from `locations.json` (researched by location-generator) |
| Invent highways/exits | Must come from `locations.json` |
| Skip reading `locations.json` | This file contains the real local facts |
| Skip reading `state-compliance/*.json` | This contains YMYL regulatory data |

### ✅ YOU MUST:

1. **FIRST** read `locations.json` to get local facts for assigned cities
2. **THEN** read `state-compliance/[STATE].json` for regulatory content
3. **THEN** read service list for internal linking
4. **USE** the data from these files - not your memory

### 🔍 VERIFICATION:

Before writing any city page JSON:
- You used `Read` tool on `locations.json`
- Landmarks in output match landmarks from `locations.json`
- Highways in output match highways from `locations.json`
- State compliance content matches `state-compliance/*.json`

**If landmarks/highways don't match input files, you're generating from memory - DELETE and start over.**
...
## Database Connection (Financial Stats)

**Global Cache Database**: `localhost:13247` (PostgreSQL)

The `uscities` table contains financial/demographic data for US cities:

```sql
-- Schema
city TEXT,                    -- e.g., "Los Angeles"
state_id TEXT,               -- e.g., "CA"
population INTEGER,
...
**Query to fetch financial stats for a city:**
```bash
PGPASSWORD=password psql -h localhost -p 13247 -U postgres -d cities_db -t -A -F',' -c "
SELECT
...
FROM uscities
WHERE city = 'Los Angeles' AND state_id = 'CA'
LIMIT 1;"
```

**Use financial stats for:**
1. **Market Context** - "With median income of $X in {City}, many residents..."
2. **Local Relevance** - Shows understanding of local economic conditions
3. **E-E-A-T Signals** - Real data demonstrates expertise
4. **Anti-Doorway** - Unique, factual content that can't be mass-generated

## Why Anti-Doorway Matters

> See `docs/ANTI-DOORWAY.md` for full philosophy and requirements.
...
**4. Fetch Financial Stats from Database (NEW)**

For each city, query the `uscities` table for economic data:

```bash
# Fetch financial stats for Los Angeles, CA
...
**If data exists**, add to city page JSON:
```json
{
  "financialStats": {
    "medianIncome": 65290,
    "unemploymentRate": 5.2,
    "medianHomeValue": 742800,
    "medianRent": 1723,
    "povertyRate": 13.4,
    "laborForceParticipation": 64.2,
    "population": 3898747
  },
  "marketContext": "With a median household income of $65,290, Los Angeles residents may need quick access to cash for unexpected expenses."
}
```

**If no data found**, omit the `financialStats` field (don't fail).
